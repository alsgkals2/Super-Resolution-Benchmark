...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        self.file_name = input_img if type(input_img) is str else conf.name #default = zssr_gradual
    ...
    def run(self):
        ...
        for self.sf_ind, (sf, self.kernel) in enumerate(zip(self.conf.scale_factors, self.kernels)):
            ...
            filename = os.path.basename(self.file_name)[:-4]
            if os.path.exists(f'{self.conf.result_path}/{self.conf.name_dataset}/{filename}_zssr_{sf_str}.png'):
                print(f'Already Exists: {self.conf.result_path}/{self.conf.name_dataset}/{filename}_zssr_{sf_str}.png')
                continue
            ...
            if self.conf.save_results:
                plt.imsave(f'{self.conf.result_path}/{self.conf.name_dataset}/{filename}_zssr_{sf_str}.png',post_processed_output,vmin=0, vmax=1)
    ...
    def init_sess(self, init_weights=True):
        ...
        print(f'gt_per_sf size : {self.gt_per_sf.shape}')
    ...
    def forward_pass(self, lr_son, hr_father_shape=None):
        ...
        print(f'interpolated_lr_son size : {interpolated_lr_son.shape}')
        # Create feed dict
    ...
    def quick_test(self):
        ...
        # print(self.gt_per_sf.shape , self.sr.shape)
    ...
    def train(self):
        # main training loop
        for self.iter in tqdm.tqdm(range(self.conf.max_iters)):
            ...
            # print(f'lr_son is {self.lr_son.shape}') #(128,86)
            # print(f'hr_father is {self.hr_father.shape}') #(128,128)
            # print(f'train_output is {self.train_output.shape}') #(128,128)
            ...
            if self.learning_rate < self.conf.min_learning_rate:
                print(f"Stop train -------> ep:{self.iter} | lr:{self.learning_rate}")
                break
    ...
    def final_test(self):
        ...
        for k in tqdm.tqdm(range(0, 1 + 7 * self.conf.output_flip, 1 + int(self.sf[0] != self.sf[1]))):
